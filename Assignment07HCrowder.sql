...
-- 2022-03-08,HeatherCrowder,Created File
-- 2022-03-16,HeatherCrowder, Completed File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_HeatherCrowder')
	 Begin 
	  Alter Database [Assignment07DB_HeatherCrowder] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_HeatherCrowder;
	 End
	Create Database Assignment07DB_HeatherCrowder;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_HeatherCrowder;
...
/*SELECT CategoryName, 
 FORMAT (InventoryDate, 'MMMM, yyyy') AS InventoryDate,
    Count AS InventoryCountByCategory
...
FORMAT (InventoryDate, 'MMMM, yyyy') AS InventoryDate, 
Count AS InventoryCount, 
LAG(Count) OVER (ORDER BY ProductName, DATEPART (Month, InventoryDate)) AS PreviousMonthCount
    FROM dbo.Products as P
      JOIN dbo.Inventories as I
	    ON P.ProductID = I.ProductID
    ORDER BY ProductName, DATEPART (Month, InventoryDate);
